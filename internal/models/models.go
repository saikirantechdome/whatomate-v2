...
type Template struct {
	...
	HeaderMediaID   string     `gorm:"size:255" json:"header_media_id"` // WhatsApp media ID for IMAGE/VIDEO/DOCUMENT headers, used when sending (HeaderContent only holds Meta's one-time upload handle, valid for template creation only)
